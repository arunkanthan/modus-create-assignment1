...
Budget Page
-----------
1. User is able to add to the budget list and list is updated
2. Total Inflow and Total Outflow and working balance are updated as items are added to the list
3. Total Inflow and Total Outflow and working balance are zero when budget list is empty
4. Add button is disabled if any input field is empty
5. Value field accepts only numbers
6. OUtflow entries are highlighted in red.
7. Inflow entries are highlighted in green
8. User is able to open and close category list by click the dropdown
9. Vertical scroll bar is displayed if entries exceed avaialble space
10. The sum or difference calculated between total inflow and total outflow is correct.  
11. Verify UI elements are displayed properly on zoom-in and zoom-out
12. Budgeting home page is displayed when user navigates to "hostname"<port>/budget"
13. Verify Budget app takes only valid inputs in english(lcoalization is not supported)
14. Adding too many entries should not crash the application
15. verify inputting junk values into any input field throws error or is not accepted.
16. verify budget title is highlighted when clicked on it and budget page is opened.

Reports Page
-----------
1. User can navigate to reports page by clicking on reports link
2. Reports page dispalys two tabs "Inflow vs Outflow" and "Spending By category"
3. Clicking on "Inflow vs Outflow" shows bar graphs of total inflow and outflow, outflow bar is categorized 
in different colors as per category
3. Clicking on "Spending by category" shows pie chart with different category spending
4. Verify UI elements are displayed properly on zoom-in and zoom-out
5. User is able to switch back and forth between "Budget" page and "reports" page
6. Update budget list and verify the changes are reflected in reports page
7. verify reports title is highlighted when clicked on it and reports page is opened.


Non Functional tests
--------------------
1. verify webpage loads within acceptable time frame (as per industry standards)
2. verify dropdown list shows up within acceptable time frame
3. verify there is no resource leak on running applciation continuosly for more than 24 hrs 
4. verify app inout fields are not suceptible to Javascript injection adn XSS attacks 
